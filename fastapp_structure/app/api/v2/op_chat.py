...
# def _safe_read(path: str) -> str:
#     try: return open(path,"r",encoding="utf-8",errors="ignore").read().strip()
#     except: return ""
...
#     for name in os.listdir(base):
#         d=os.path.join(base,name)
#         if not (os.path.isdir(d) and _has_faiss(d)): continue
#         text=""
#         meta=os.path.join(d,"meta.json")
#         if os.path.exists(meta):
#             try: text=json.loads(_safe_read(meta) or "{}").get("description","")
#             except: pass
#         if not text:
...
#                     text=_safe_read(fp); 
#                     if text: break
#         if not text: text=name.replace("_"," ").replace("-"," ")
#         descs[name]=text
#     return descs
...
#         v=np.array(v,dtype="float32"); sim=float(np.dot(q,v)/(np.linalg.norm(v) or 1.0)/qn)
...
#     prev_qa_block=(f"Previous QA context:\nQ:{prev_qa['question']}\nA:{prev_qa['answer']}\n—End—") if prev_qa else ""

#     def with_prev(q:str)->str:
#         return q if not prev_qa else f"{q}\n\n[Disambig]\nPrevQ:{prev_qa['question']}\nPrevA:{prev_qa['answer']}"



#     # Build a tiny "previous QA" context (only one pair) for coreference resolution.
#     prev_qa = get_last_qa_pair(history)
#     prev_qa_block = ""
#     if prev_qa:
#         prev_qa_block = (
...
#     # Helper to augment a question with the last QA (only if present)
#     def with_prev_qa(q: str) -> str:
#         if not prev_qa:
#             return q
#         return (
#             f"{q}\n\n[Context for disambiguation]\n"
#             f"Previous Question: {prev_qa['question']}\n"
#             f"Previous Answer: {prev_qa['answer']}\n"
#             f"[End context]"
#         )
...
#         # <<< inject prev QA into question
#         mongo_query = generate_ai_mongo_query_with_fallback(with_prev_qa(request.question), username, context_info)
...
#     # Step 1: LLM classification (pass previous QA as system context if present)
#     messages = [{"role": "system", "content": DEFAULT_SYSTEM_PROMPT}]
#     if prev_qa_block:
#         messages.append({"role": "system", "content": prev_qa_block})
...
#                 # tags are explicit (no need to inject prev QA), keep as-is:
...
#             # <<< inject prev QA into the query text for FAISS routing
#             best_index_name = pick_best_index(with_prev_qa(request.question))  # fast + cached
...
#                     kb_snippets = query_documents(with_prev_qa(request.question), index_path)
